# inCart Platform Code Technique and Pattern

Summarize all technique and pattern use in inCart Platform Project

## Table of Contents
* [JS](JS.md)
  * [Components](JS.md#components)
  * [HOCs](JS.md#hocs)
* [JSON](JSON.md)
  * [\_meta](JSON.md#_meta)
    * [hoc/hocs](JSON.md#hochocs)
    * [context](JSON.md#context)
    * [translate](JSON.md#translate)
    * [slots](JSON.md#slots)
    * [style](JSON.md#style)
    * [hiddenOn](JSON.md#hiddenon)
    * [useTheme](JSON.md#usetheme)
    * [cautions](JSON.md#cautions)
* [CSS](CSS.md)
  * [META inline](CSS.md#meta-inline)
  * [CMS style](CSS.md#cms-style)
    * [inCart Component](CSS.md#incart-component-style)
    * [Semantic Component](CSS.md#semantic-component-style)
  * [Component style](CSS.md#component-style)
    * [CSS](CSS.md#css)
    * [Theming](CSS.md#theming)
  * [Global style](CSS.md#global-style)
